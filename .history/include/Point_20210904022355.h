...
namespace Curves
{
    ...
    public:
        ...
        friend P operator+(const P &left, const P &right);
        friend P operator*(const P &left, const P &right);
        friend P operator*(const P &left, const T right);
...
namespace Curves{
    ...
    template <typename T>
    void P::setPoint(T x1, T x2){
        point[0] = x1;
        point[1] = x2;
    }

    template<typename T>
    void P::printPoint() const{
        std::cout << '(' << point[0] << ", " << point[1] << ')' << '\n';
    }
}

P P::operator+(const P &left, const P &right);
